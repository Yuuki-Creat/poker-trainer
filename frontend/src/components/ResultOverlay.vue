<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
    <div class="bg-white w-full max-w-sm rounded-3xl p-8 text-center shadow-2xl transform transition-all scale-100">
      ...
      <p class="text-sm text-slate-500 mb-6 leading-relaxed">{{ result.explanation }}</p>
      
      <button @click="$emit('next')" class="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 active:scale-95 transition-all">
        Next Hand
      </button>
    </div>
  </div>
